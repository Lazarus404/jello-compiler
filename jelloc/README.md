...
Jello’s reference compiler, used to **prove** the VM end-to-end by compiling a small, statically typed, inference-friendly language to Jello bytecode.

Updated to Rust

## Compiler architecture (high level)

`jelloc` targets a **register-based VM with statically typed registers** and a **boxed-only spill stack**. The compiler is structured to keep the IR and bytecode aligned with those VM invariants:
...
From the project directory in a checkout that includes the VM at **`../jellovm`** (default `embed-vm` links `libjellovm` via CMake):
...
```bash
cargo run --manifest-path jelloc/Cargo.toml -- --help
```
