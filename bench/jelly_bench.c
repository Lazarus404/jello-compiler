...
static jello_bc_result make_loop_module(uint32_t n, jello_bc_module** out) {
  ...
  return jello_bc_read(buf, off, out);
}

static jello_bc_result make_todyn_fromdyn_loop_module(uint32_t n, jello_bc_module** out) {
...
static jello_bc_result make_array_i32_module(uint32_t n, jello_bc_module** out) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_ARRAY, 0); // p0=elem type_id (I32)
...
static jello_bc_result make_bytes_u8_module(uint32_t n_in, jello_bc_module** out, uint32_t* used_n) {
...
static jello_bc_result make_kindof_switch_module(uint32_t n, jello_bc_module** out) {
...
int main(int argc, char** argv) {
  ...
  for(uint32_t i = 0; i < runs; i++) {
    ...
    if(jello_is_i32(out)) last = (int64_t)jello_as_i32(out);
    jello_vm_destroy(vm);
  }
  uint64_t t1 = now_ns();
  ...
  printf("jello_bench %s n=%u runs=%u: total=%.2fms (%.2fus/run) result=%lld\n",
