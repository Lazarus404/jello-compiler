# Jello VM performance notes

This document records Jello’s performance goals, the design choices made to support them, and what to compare against when we benchmark.
...
## What makes Jello fast (by design)
...
- **Boxing only at boundaries (Jello-style)**:
  - `Dynamic` is a boxed/tagged `jello_value`.
...
- **Lua 5.4**:
  ...
  - Dynamic typing implies more runtime tagging and type checks than Jello’s typed registers.
...
- **Interpreter loop** in `jellovm/src/vm/interp.c` + `jellovm/src/vm/ops/exec_loop.c`
