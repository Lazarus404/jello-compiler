...
This document describes how the Jello REPL works and the plan for further improvements.
...
- **`jelloc`** (no args) or **`jelloc --repl`** — Start REPL with no preload
- **`jelloc --repl <file.jello>`** — Start REPL with `<file.jello>` preloaded and executed once

### Execution Modes

| Mode                   | When                       | Behavior                                                   |
| ---------------------- | -------------------------- | ---------------------------------------------------------- |
| **Embed VM** (default) | `embed-vm` feature enabled | jelloc links `libjellovm`; runs VM in-process via `ReplVm` |
| **Subprocess**         | `--no-default-features`    | jelloc spawns `jellovm` subprocess per line                |
...
3. **Execute**: `repl_vm.exec(bytecode)` (embed) or spawn jellovm (subprocess)
...
| `session_exports` | Phase 2: exports object from last run (JelloValue; for future use)    |
...
- **Heap**: Phase 2: heap persists across runs (no `jello_gc_shutdown` between runs).
- **Exports capture**: `jello_vm_exec_status_exports` captures the entry module's exports object on top-level return; stored in `session.session_exports` for future incremental exec.
...
With `--repl file.jello`, the file is compiled and run once before the prompt. Its bindings and artifact seed the session.
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  jelloc (single process, links libjellovm)                                      │
│                                                                                 │
│   ┌──────────────┐     ┌──────────────────────┐     ┌──────────────────────────┐│
...
```
┌─────────────────────────────────────────────────────────────────────────────────┐
│  jelloc REPL loop (--no-default-features)                                       │
│    ┌──────────────┐     ┌──────────────┐     ┌─────────────┐                    │
...
**Implemented** (2025-03): jelloc links `libjellovm` via `embed-vm` feature (default). REPL runs VM in-process; no subprocess. Use `--no-default-features` to fall back to subprocess (requires jellovm binary).

| Step | Task                            | Notes                                                                                 |
| ---- | ------------------------------- | ------------------------------------------------------------------------------------- |
| 1.1  | Build VM as static lib          | CMake target for `libjellovm.a`; ensure jelloc build system can link it               |
| 1.2  | Rust FFI bindings               | `jello_vm_new`, `jello_vm_load`, `jello_vm_exec_status`, `jello_value`, trap handling |
| 1.3  | Prelude loading                 | Compile prelude at build time or load `prelude.jlo`; pass to VM once at REPL start    |
| 1.4  | Replace `Command::new(jellovm)` | Call VM APIs directly from `repl.rs`                                                  |
| 1.5  | Result printing                 | Convert `jello_value` to string in Rust (or call VM helper)                           |
...
| Step | Task                               | Notes                                                                                            |
| ---- | ---------------------------------- | ------------------------------------------------------------------------------------------------ |
| 2.1  | VM: support "resumable" execution  | ✅ **Done**: Heap persists; `jello_gc_shutdown` no longer called between runs                    |
| 2.2  | VM: `jello_vm_exec_status_exports` | ✅ **Done**: Captures entry module's exports on top-level return; `entry_module_idx` 0 or 1      |
...
| Step | Task                             | Notes                                                                         |
| ---- | -------------------------------- | ----------------------------------------------------------------------------- |
| 4.1  | VM: exec with pre-bound args     | ✅ **Done**: `jello_vm_exec_status_chunk`, `vm_push_frame_from_values`        |
...
**Implementation**: Chunk uses the same JLO format. `link_chunk_for_repl` produces a module whose entry takes 1 arg (session_exports), creates exports, and calls init(exports, session_exports). When `session_exports` is set and incremental succeeds, REPL uses `exec_chunk` instead of full linked exec.
...
4. **Preload (`--repl file.jello`)**: Preload runs once; session is seeded with its artifact and bindings; REPL extends that env.
...
| Purpose                      | Path                                                                                   |
| ---------------------------- | -------------------------------------------------------------------------------------- |
| REPL entry, session, compile | `jelloc/src/repl.rs`                                                                   |
| In-process VM (embed-vm)     | `jelloc/src/vm.rs`                                                                     |
| REPL binding collection      | `jelloc/src/link/collect.rs` (`collect_repl_bindings`, `collect_free_vars_in_program`) |
