# Jello Compiler Architecture

This document describes the structure of the Jello compiler (`jelloc`) and how its components relate. The compiler is written in Rust and produces Jello bytecode (`.jlo`) for the Jello VM.
...
│  - vstream → allocate → finalize → jlo::Module                             │
└─────────────────────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
| `jlo`       | Bytecode format, prelude, validation                                     |
...
- For bytecode: read `.jlo`, extract ABI
...
Source
  ...
  → codegen (vstream → allocate → finalize) → jlo::Module
  → link → final bytecode
```
...
| Purpose                   | Path                                            |
| ------------------------- | ----------------------------------------------- |
| Compilation orchestration | `jelloc/src/compile.rs`                         |
| Frontend pipeline         | `jelloc/src/frontend.rs`                        |
| Parse entry               | `jelloc/src/parse/mod.rs`                       |
| Semantic entry            | `jelloc/src/semantic/entrypoint/mod.rs`         |
| Typecheck entry           | `jelloc/src/semantic/typecheck/mod.rs`          |
| Lower entry               | `jelloc/src/lower/entry.rs`                     |
| IR model                  | `jelloc/src/ir/model.rs`, `jelloc/src/ir/op.rs` |
| Opt pipeline              | `jelloc/src/opt/mod.rs`                         |
| Codegen entry             | `jelloc/src/codegen/entry.rs`                   |
| VStream                   | `jelloc/src/codegen/vstream.rs`                 |
| Link load                 | `jelloc/src/link/load.rs`                       |
| Linker                    | `jelloc/src/link/linker.rs`                     |
