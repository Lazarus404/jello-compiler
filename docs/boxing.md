...
Jello is a **typed VM** with an explicit boundary between:

- **typed registers** (unboxed, type known from bytecode), and
- **`Dynamic` values** (boxed/tagged `jello_value`).
...
- **`Dynamic` is self-describing** via `jello_value` tagging + heap object headers.
...
### `jello_value` tags (Jello-style)

`jello_value` is a `uintptr_t` using low-bit tags:

- **`null`**, **`bool`**, **`i32`** etc., **`atom`**: immediate tagged values
- **pointers**: heap objects (tag `PTR`), identified by `jello_obj_header.kind`

Wide numerics are boxed as heap objects so tagging stays portable:

- `I64` -> `JELLO_OBJ_BOX_I64`
- `F64` -> `JELLO_OBJ_BOX_F64`
- `F32` -> `JELLO_OBJ_BOX_F32`
...
- pointer kinds (`Bytes/Function/List/Array/Object/Abstract`) -> `null` or pointer `jello_value`
- `Dynamic` -> copy
...
- `jello_gc_push_root(vm, value)`
- `jello_gc_pop_roots(vm, n)`
