## Jello call ABI and capture layout (compiler ↔ VM contract)

This document describes the **register layout invariants** that `jelloc` must respect when emitting bytecode for the Jello VM.
...
In codegen, `jelloc` allocates one contiguous physical **arg block** per signature and _pins_ each call’s argument window vregs to the corresponding physical block.
...
- If the module enables **`CAP_START`** (`JELLO_BC_FEAT_CAP_START`), the VM copies captures starting at `callee.cap_start`.
...
- `jelloc/src/codegen/phase2.rs` (pinning and invariants)
