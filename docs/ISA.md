...
This is the canonical reference for Jello’s bytecode container format and opcode semantics.
...
- **Dynamic rule**: `Dynamic` registers store boxed `jello_value` (Jello-style tagging); typed registers are unboxed.

### Bytecode container format (`JLO`, version 1)

All integers are **little-endian**.

#### Header

- `u32 magic` = `'JLO'` (`0x4A4C5942`)
- `u32 version` = `1`
- `u32 features` = bitset (`jello_bc_feature_flags`)
- `u32 ntypes`
- `u32 nsigs`
- `u32 natoms`
- `u32 nfuncs`
...
`kind` is `jello_type_kind` (see `jellovm/src/include/jello.h`).
...
#### Optional 64-bit const pools (`JELLO_BC_FEAT_CONST64`)
...
#### Optional bytes const pool (`JELLO_BC_FEAT_CONSTBYTES`)
...
- Spill stack exists for register pressure, but **spill holds boxed `jello_value` only**.
...
- **`JOP_CONST_I64 a imm`**:
  ...
  - requires `features & JELLO_BC_FEAT_CONST64`
...
- **`JOP_CONST_BYTES a imm`**:
  ...
  - requires `features & JELLO_BC_FEAT_CONSTBYTES`
...
When a trap is caught, the catch payload is a boxed `i32` equal to the `jello_trap_code`.
...
On disk (in `jello_bc_read` format), atoms are encoded as:
