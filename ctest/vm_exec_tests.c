...
/* Logical func index: 0..JELLO_NATIVE_BUILTIN_COUNT-1 = native, then bytecode funcs. */
#define BC_FUNC_IDX(i) (JELLO_NATIVE_BUILTIN_COUNT + (i))
#include <stdlib.h>
#include <string.h>
...
static void assert_i32_eq(const char* test, jello_value v, int32_t want, const char* msg) {
  if(!jello_is_i32(v) || jello_as_i32(v) != want) {
    failf(test, msg);
  }
}

static void assert_bool_eq(const char* test, jello_value v, int want, const char* msg) {
  if(!jello_is_bool(v) || (int)jello_as_bool(v) != want) {
    failf(test, msg);
  }
}
...
static int run_exec_add_i32(void) {
  ...
  // header
  wr_u32(buf + off, JELLO_BC_MAGIC); off += 4;
  ...
  // types[0] = I32
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  ...
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_OK, "module loads");
  ...
  jello_value v = jello_vm_exec(vm, m);
...
static int run_exec_i32_imm_cmp(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0);
  ...
  jello_vm* vm = jello_vm_create();
  jello_value v = jello_vm_exec(vm, m);
  assert_true(t, jello_is_bool(v) && jello_as_bool(v) == 1, "eq_imm and lt_imm yield true");
...
static int run_exec_lt_f64(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);  /* 0 */
  wr_type(buf, &off, (uint8_t)JELLO_T_F64, 0);  /* 1 */
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0); /* 2 */
...
static int run_exec_lt_f32(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);  /* 0 */
  wr_type(buf, &off, (uint8_t)JELLO_T_F32, 0);  /* 1 */
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0); /* 2 */
...
static int run_exec_lt_i64(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);  /* 0 */
  wr_type(buf, &off, (uint8_t)JELLO_T_I64, 0);  /* 1 */
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0); /* 2 */
...
static int run_exec_const_bytes_len_and_get_u8(void) {
  ...
  wr_u32(buf + off, JELLO_BC_MAGIC); off += 4;
  wr_u32(buf + off, 1); off += 4; // version
  wr_u32(buf + off, (uint32_t)JELLO_BC_FEAT_CONSTBYTES); off += 4; // features
  ...
  // types: bytes (0), i32 (1)
  wr_type(buf, &off, (uint8_t)JELLO_T_BYTES, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
...
static int run_exec_bytes_concat_many_and_len(void) {
  ...
  wr_u32(buf + off, JELLO_BC_MAGIC); off += 4;
  wr_u32(buf + off, 1); off += 4;
  wr_u32(buf + off, (uint32_t)JELLO_BC_FEAT_CONSTBYTES); off += 4;
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_BYTES, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_ARRAY, 0);
...
static int run_exec_spill_roundtrip(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_DYNAMIC, 0);
...
static int run_exec_list_head_tail(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_LIST, 0); // elem type id = 0
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0);
...
static int run_exec_list_is_nil(void) {
  ...
  jello_vm* vm = jello_vm_create();
  jello_value v = jello_vm_exec(vm, m);
  assert_true(t, jello_is_bool(v) && jello_as_bool(v) == 1, "nil list is_nil => true");
...
static int run_exec_array_set_get_len(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_ARRAY, 0); // elem = 0
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0);
...
static int run_exec_array_len(void) {
  ...
  assert_true(t, st == JELLO_EXEC_OK, "exec returns OK");
  if(st != JELLO_EXEC_OK) {
    fprintf(stderr, "[TRAP] %s: code=%u msg=%s\n", t, (unsigned)jello_vm_last_trap_code(vm), jello_vm_last_trap_msg(vm) ? jello_vm_last_trap_msg(vm) : "(null)");
    jello_vm_destroy(vm);
    jello_bc_free(m);
    return 1;
  }
...
static int run_exec_obj_get_atom(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_OBJECT, 0);
...
static int run_exec_obj_has_atom(void) {
  ...
  jello_vm* vm = jello_vm_create();
  jello_value v = jello_vm_exec(vm, m);
  assert_true(t, jello_is_bool(v) && jello_as_bool(v) == 1, "object has_atom => true");
...
static int run_exec_call_indirect_i32(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_FUNCTION, 0);
...
static int run_exec_closure_capture_i64_aggressive_gc(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_I64, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_FUNCTION, 0);
...
static int run_gc_cycle_reclaim_object(void) {
  ...
  jello_object* a = jello_object_new(vm, 0);
  // Root during setup to prevent GC-triggered reclamation.
  jello_gc_push_root(vm, jello_from_ptr(a));
  jello_object* b = jello_object_new(vm, 0);
  jello_gc_push_root(vm, jello_from_ptr(b));
  jello_object_set(a, 1, jello_from_ptr(b));
  jello_object_set(b, 2, jello_from_ptr(a));
...
static int run_gc_abstract_finalizer_called_once(void) {
  ...
  *abs_finalized = 0;
  ...
  jello_abstract* a = jello_abstract_new_finalized(vm, 0, abs_finalized, abs_finalizer_inc);
...
static int run_gc_mixed_container_graph_reclaim(void) {
  ...
  jello_object* obj = jello_object_new(vm, 0);
  jello_gc_push_root(vm, jello_from_ptr(obj));

  jello_array* arr = jello_array_new(vm, 0, 1);
  jello_gc_push_root(vm, jello_from_ptr(arr));

  jello_box_i64* b = jello_box_i64_new(vm, 0, 123);
  jello_value vb = jello_from_ptr(b);
  jello_gc_push_root(vm, vb);

  jello_list* list = jello_list_cons(vm, 0, vb, NULL);
  jello_gc_push_root(vm, jello_from_ptr(list));

  // Link graph.
  arr->data[0] = jello_from_ptr(obj);     // cycle back to obj
  jello_object_set(obj, 10, jello_from_ptr(arr));
  jello_object_set(obj, 11, jello_from_ptr(list));

  // Drop setup roots except the main `obj` root.
  jello_gc_pop_roots(vm, 3);
  ...
  jello_value v_arr = jello_object_get(obj, 10);
  assert_true(t, jello_is_ptr(v_arr), "arr is a pointer value");
  jello_array* arr2 = (jello_array*)jello_as_ptr(v_arr);
  assert_true(t, arr2 && arr2->h.kind == (uint32_t)JELLO_OBJ_ARRAY, "arr kind ok");

  jello_value v_obj_back = arr2->data[0];
  assert_true(t, jello_is_ptr(v_obj_back) && jello_as_ptr(v_obj_back) == (void*)obj, "cycle obj <-> arr intact");

  jello_value v_list = jello_object_get(obj, 11);
  assert_true(t, jello_is_ptr(v_list), "list is a pointer value");
  jello_list* list2 = (jello_list*)jello_as_ptr(v_list);
  assert_true(t, list2 && list2->h.kind == (uint32_t)JELLO_OBJ_LIST, "list kind ok");
  assert_true(t, jello_is_box_i64(list2->head) && jello_as_box_i64(list2->head) == 123, "boxed i64 survives");
...
static int run_gc_object_rehash_under_aggressive_gc(void) {
  ...
  for(uint32_t i = 0; i < n; i++) {
    jello_value boxv = jello_from_ptr(jello_box_i64_new(vm, 0, (int64_t)(1000 + (int64_t)i)));
    jello_gc_push_root(vm, boxv);
    jello_object_set(obj, i + 1u, boxv);
    jello_gc_pop_roots(vm, 1);
  }
...
static int run_exec_from_dyn_bool(void) {
  ...
  jello_vm* vm = jello_vm_create();
  jello_value v = jello_vm_exec(vm, m);
  assert_true(t, jello_is_bool(v) && jello_as_bool(v) == 1, "from_dyn_bool roundtrip");
...
static int run_exec_from_dyn_atom(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_ATOM, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_DYNAMIC, 0);
  ...
  jello_vm* vm = jello_vm_create();
  jello_value v = jello_vm_exec(vm, m);
  assert_true(t, jello_is_atom(v) && jello_as_atom(v) == 0u, "from_dyn_atom roundtrip");
...
static int run_exec_trap_bytes_get_oob(void) {
  ...
  jello_value out = jello_make_null();
  jello_exec_status st = jello_vm_exec_status(vm, m, &out);
  assert_true(t, st == JELLO_EXEC_TRAP, "exec returns TRAP");
  assert_true(t, jello_vm_last_trap_code(vm) == JELLO_TRAP_BOUNDS, "trap is BOUNDS");
...
static int run_exec_trap_from_dyn_bool_mismatch(void) {
  ...
  jello_exec_status st = jello_vm_exec_status(vm, m, &out);
  assert_true(t, st == JELLO_EXEC_TRAP, "exec returns TRAP");
  assert_true(t, jello_vm_last_trap_code(vm) == JELLO_TRAP_TYPE_MISMATCH, "trap is TYPE_MISMATCH");
...
static int run_exec_try_catch_catches_trap_bytes_oob(void) {
  ...
  jello_vm* vm = jello_vm_create();
  jello_value v = jello_vm_exec(vm, m);
  assert_i32_eq(t, v, (int32_t)JELLO_TRAP_BOUNDS, "caught payload is trap code");
  assert_true(t, jello_vm_last_trap_code(vm) == JELLO_TRAP_NONE, "trap cleared after catch");
...
static int run_exec_kindof_i32_and_null(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_LIST, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_DYNAMIC, 0);
...
static int run_exec_i8_i16_arith_cmp_relaxed(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I8, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_I16, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_BOOL, 0);
...
static int run_exec_f64_dyn_roundtrip(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_F64, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_DYNAMIC, 0);
...
static int run_exec_from_dyn_f32(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_F32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_DYNAMIC, 0);
...
static int run_exec_f32_dyn_and_call_return(void) {
  ...
  jello_vm* vm = jello_vm_create();
  jello_value v = jello_vm_exec(vm, m);
  assert_true(t, jello_is_box_f32(v), "returns boxed f32");
  float got = jello_as_box_f32(v);
...
static int run_exec_f16_i32_convert(void) {
  ...
  wr_type(buf, &off, (uint8_t)JELLO_T_I32, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_F16, 0);
  wr_type(buf, &off, (uint8_t)JELLO_T_F32, 0);
...
static int run_exec_const_f16_to_dyn(void) {
  ...
  assert_true(t, jello_is_ptr(v), "result is ptr (box_f16)");
  assert_true(t, jello_obj_kind_of(v) == (uint32_t)JELLO_OBJ_BOX_F16, "result is box_f16");
  assert_true(t, jello_as_box_f16(v) == 0x3C00, "f16 value is 1.0");
...
static int run_exec_i64_arith_and_eq_wrap(void) {
  ...
  wr_u32(buf + off, JELLO_BC_MAGIC); off += 4;
  wr_u32(buf + off, 1); off += 4;
  wr_u32(buf + off, (uint32_t)JELLO_BC_FEAT_CONST64); off += 4; // features (for const_i64 pool)
  wr_u32(buf + off, 3); off += 4; // ntypes
  wr_u32(buf + off, 0); off += 4; // nsigs
  wr_u32(buf + off, 0); off += 4; // natoms
  wr_u32(buf + off, 1); off += 4; // nfuncs
...
static int run_exec_trap_f64_to_i32_overflow(void) {
  ...
  wr_u32(buf + off, JELLO_BC_MAGIC); off += 4;
  wr_u32(buf + off, 1); off += 4;
  wr_u32(buf + off, (uint32_t)JELLO_BC_FEAT_CONST64); off += 4;
  wr_u32(buf + off, 2); off += 4; // ntypes
  wr_u32(buf + off, 0); off += 4;
  wr_u32(buf + off, 0); off += 4;
  wr_u32(buf + off, 1); off += 4;
  ...
  jello_value out = 0;
  jello_exec_status st = jello_vm_exec_status(vm, m, &out);
  assert_true(t, st == JELLO_EXEC_TRAP, "conversion overflow traps");
  assert_true(t, jello_vm_last_trap_code(vm) == JELLO_TRAP_TYPE_MISMATCH, "trap code is TYPE_MISMATCH");
...
static int run_exec_const_i64_pool(void) {
  ...
  wr_u32(buf + off, JELLO_BC_MAGIC); off += 4;
  wr_u32(buf + off, 1); off += 4;
  wr_u32(buf + off, (uint32_t)JELLO_BC_FEAT_CONST64); off += 4; // features
  wr_u32(buf + off, 3); off += 4; // ntypes
  wr_u32(buf + off, 0); off += 4; // nsigs
  wr_u32(buf + off, 0); off += 4; // natoms
  wr_u32(buf + off, 1); off += 4; // nfuncs
  ...
  jello_vm* vm = jello_vm_create();
  jello_value out = jello_vm_exec(vm, m);
