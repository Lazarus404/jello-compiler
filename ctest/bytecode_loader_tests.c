...
static int run_loader_minimal_ok(void) {
  ...
  // type 0: I32
  buf[off++] = (uint8_t)JELLO_T_I32; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4;
  wr_u32(buf + off, 0); off += 4;
  // type 1: Dynamic
  buf[off++] = (uint8_t)JELLO_T_DYNAMIC; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4;
  wr_u32(buf + off, 0); off += 4;
  ...
  assert_true(t, r.err == JELLO_BC_OK, "load should succeed");
...
static int run_loader_bad_magic(void) {
  ...
  wr_u32(buf + 4, 1);
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, sizeof(buf), &m);
  assert_true(t, r.err == JELLO_BC_BAD_MAGIC, "bad magic detected");
  return failures == 0 ? 0 : 1;
}
...
static int run_loader_nregs_too_big(void) {
  ...
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_BAD_FORMAT, "nregs>256 rejected");
  return failures == 0 ? 0 : 1;
}
...
static int run_loader_bad_insn_type_mismatch(void) {
  ...
  buf[off++] = (uint8_t)JELLO_T_BOOL; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4; wr_u32(buf + off, 0); off += 4;
  ...
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_BAD_FORMAT, "type mismatch rejected");
  return failures == 0 ? 0 : 1;
}
...
static int run_loader_const_bytes_roundtrip(void) {
  ...
  // type0: bytes
  buf[off++] = (uint8_t)JELLO_T_BYTES; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4; wr_u32(buf + off, 0); off += 4;
...
static int run_loader_const_bool_bad_imm(void) {
  ...
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_BAD_FORMAT, "bad bool imm rejected");
  return failures == 0 ? 0 : 1;
}
...
static int run_loader_const_i8_oob(void) {
  ...
  // type0: I8
  buf[off++] = (uint8_t)JELLO_T_I8; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4; wr_u32(buf + off, 0); off += 4;
  ...
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_BAD_FORMAT, "i8 range enforced on const_i32");
  return failures == 0 ? 0 : 1;
}
...
static int run_loader_const_i16_oob(void) {
  ...
  // type0: I16
  buf[off++] = (uint8_t)JELLO_T_I16; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4; wr_u32(buf + off, 0); off += 4;
  ...
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_BAD_FORMAT, "i16 range enforced on const_i32");
  return failures == 0 ? 0 : 1;
}
...
static int run_loader_atom_id_oob(void) {
  ...
  buf[off++] = (uint8_t)JELLO_T_ATOM; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4; wr_u32(buf + off, 0); off += 4;
  buf[off++] = (uint8_t)JELLO_T_OBJECT; buf[off++] = 0; buf[off++] = 0; buf[off++] = 0;
  wr_u32(buf + off, 0); off += 4; wr_u32(buf + off, 0); off += 4;
  ...
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_BAD_FORMAT, "atom id bounds enforced");
  return failures == 0 ? 0 : 1;
}
...
static int run_loader_jmp_target_oob(void) {
  ...
  jello_bc_module* m = NULL;
  jello_bc_result r = jello_bc_read(buf, off, &m);
  assert_true(t, r.err == JELLO_BC_BAD_FORMAT, "jmp target bounds enforced");
  return failures == 0 ? 0 : 1;
}
